...
class Collision {
  ...
  boolean playerWalls() {
    ...
    if (hittingWall) { 
      velocity.y = 0.0;
    }     // This stops the player from passing through the wall.
  ...
  void enemyCollision() {
    // Enemy collision with the player needs to go here. Needs to be called after the players boundaries are filled in, this way the 
    // collision with the enemies will be better as the four corners of the player has already been worked out and stored and the 
    // enemies borders will be worked out previously too
  }
}
